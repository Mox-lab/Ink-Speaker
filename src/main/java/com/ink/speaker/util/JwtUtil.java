 ...
 * <p>配置项来自 application.yml 的 ink-speaker.jwt.* :</p>
 * <ul>
 *   <li>secret           : HMAC 密钥(要求 ≥ 32 字节)</li>
 *   <li>issuer           : 签发者(ink-speaker)</li>
...
public class JwtUtil {
    ...
    public JwtUtil(
            ...
            @Value("${ink-speaker.jwt.issuer:ink-speaker}") String issuer,
            @Value("${ink-speaker.jwt.access-token-ttl:PT2H}") Duration accessTtl,
            @Value("${ink-speaker.jwt.refresh-token-ttl:P7D}") Duration refreshTtl) {
        // 启动时强制校验:禁止空密钥与短密钥(HMAC-SHA256 要求 ≥ 32 字节)
        // 这是从 application.yml 移除兜底值后的失败快速通道
        if (secret == null || secret.isBlank()) {
            throw new IllegalStateException(
                    "ink-speaker.jwt.secret 未配置:请通过环境变量 JWT_SECRET 注入(≥ 32 字符)");
        }
        if (secret.getBytes(StandardCharsets.UTF_8).length < 32) {
            throw new IllegalStateException(
                    "ink-speaker.jwt.secret 长度不足:HMAC-SHA256 要求密钥 ≥ 32 字节,当前 "
