 ...
 *   <li>TTL 默认 24h,通过 {@code ink-speaker.cache.llm-cache-ttl} 调整</li>
 * </ul>
 *
 * <p>使用方式:</p>
 * <pre>
...
public class LlmCacheConfig {
     ...
     * </ul>
     *
     * @param maxWeightBytes 缓存总权重上限(字节),来自配置 {@code ink-speaker.cache.max-weight-bytes}
     * @param ttl            单条 TTL,来自配置 {@code ink-speaker.cache.llm-cache-ttl}(ISO-8601)
     * @return Caffeine CacheManager
     */
    @Bean
    @Primary
    public CacheManager cacheManager(
            @Value("${ink-speaker.cache.max-weight-bytes:52428800}") long maxWeightBytes,
            @Value("${ink-speaker.cache.llm-cache-ttl:PT24H}") java.time.Duration ttl) {
