...
public class ToolCacheConfig {
     ...
     * 工具缓存管理器。
     *
     * @param maxSize      最大条目数(每个缓存独立计数),来自 {@code ink-speaker.cache.tool-max-size}
     * @param ttlSeconds   TTL 秒,来自 {@code ink-speaker.cache.tool-ttl-sec}
    ...
    public CacheManager toolCacheManager(
            @Value("${ink-speaker.cache.tool-max-size:2000}") int maxSize,
            @Value("${ink-speaker.cache.tool-ttl-sec:300}") long ttlSeconds) {
