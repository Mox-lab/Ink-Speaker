...
public class EmbeddingConfig {
    ...
    public EmbeddingStore<TextSegment> embeddingStore(
            ...
            @Value("${ink-speaker.pgvector.table}") String table,
            @Value("${ink-speaker.pgvector.dimension}") int dim,
            @Value("${ink-speaker.pgvector.use-index}") boolean useIndex,
            @Value("${ink-speaker.pgvector.index-list-size:100}") int indexListSize) {
