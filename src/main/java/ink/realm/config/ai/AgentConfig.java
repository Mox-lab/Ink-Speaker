package ink.realm.config.ai;

import ink.realm.ai.agent.*;
import ink.realm.ai.agent.*;
import ink.realm.ai.core.director.ReviewAgent;
import ink.realm.ai.core.memory.ContextCompactor;
import ink.realm.ai.core.memory.L4jTokenCountEstimator;
import ink.realm.ai.core.memory.SanitizingChatMemory;
import ink.realm.ai.core.memory.TokenBudgetEstimator;
 ...
 * 配合 {@code ink.memory.token-budget} 精准控制上下文大小。
 * 配合 {@code ink.memory.compact-target} 由 {@link ContextCompactor}
...
public class AgentConfig {
     ...
     *
     * <p>窗口大小由 {@code ink.memory.token-budget} 控制(默认 6000),
    ...
    public ChatMemoryProvider chatMemoryProvider(
            @Value("${ink.memory.token-budget:6000}") int tokenBudget) {
        L4jTokenCountEstimator l4jEstimator = new L4jTokenCountEstimator(tokenBudgetEstimator);
        return memoryId -> new SanitizingChatMemory(
    ...
    public LoreAgent loreAgent(
            ...
            @Value("${ink.rag-top-k:5}") int topK) {
    ...
    /** 续写建议 Agent(BASE-12,无 Memory/Tools)。 */
    @Bean
    public ContinuationAgent continuationAgent(ChatModel chatModel) {
        return AiServices.builder(ContinuationAgent.class)
                .chatModel(chatModel)
                .build();
    }
}
