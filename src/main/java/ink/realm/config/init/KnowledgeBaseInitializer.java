...
public class KnowledgeBaseInitializer implements CommandLineRunner {
    ...
    @Value("${ink.knowledge-base-dir:./knowledge-base}")    // 知识库文档目录(与 application.yml 中 ink.* 配置对齐)
    private String knowledgeBaseDir;

    @Value("${ink.pgvector.table:langchain4j_embeddings}")  // pgvector 向量表名(与 EmbeddingConfig 中保持一致)
    ...
    public void run(String @NonNull ... args) {
        ...
        try {
            ...
            existing = jdbc.queryForObject("SELECT COUNT(1) FROM " + vectorTable, Long.class);
        } catch (Exception e) {
